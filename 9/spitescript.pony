...
class _Stack
  ...
  fun ref pick() ? =>
    let n = pop()?.usize()
    let x = _stack(_stack.size() - n - 1)?
    _stack.push(x)

  fun ref roll() ? =>
    let n = pop()?.usize()
    let x = _stack(_stack.size() - n - 1)?
    for i in Range(_stack.size() - n - 1, _stack.size() - 1) do
      _stack(i)? = _stack(i + 1)?
    end
    _stack.pop()?
    _stack.push(x)
...
class Floor is PostSpiteInstruction
  var _next: PostSpiteInstruction box
  ...
  fun exec(dmachine: PostSpiteMachine): (PostSpiteInstruction box | PostSpiteError) =>
    try
      let x = dmachine.pop()?
      dmachine.push(x.floor())
      _next
    else
      PostSpiteError("floor: not enough arguments on stack")
    end
...
class Roll is PostSpiteInstruction
  var _next: PostSpiteInstruction box
  ...
  fun exec(dmachine: PostSpiteMachine): (PostSpiteInstruction box | PostSpiteError) =>
    try
      ...
      PostSpiteError("roll: not enough arguments on stack")
    end

class Pick is PostSpiteInstruction
  var _next: PostSpiteInstruction box
  ...
  fun exec(dmachine: PostSpiteMachine): (PostSpiteInstruction box | PostSpiteError) =>
    try
      ...
      PostSpiteError("pick: not enough arguments on stack")
    end
...
class Call is PostSpiteInstruction
  ...
  fun exec(dmachine: PostSpiteMachine): (PostSpiteInstruction box | PostSpiteError) =>
    try
      ...
      PostSpiteError("call: incorrect arguments")
    end
...
class QuadAbs is PostSpiteInstruction
  ...
  fun exec(dmachine: PostSpiteMachine): (PostSpiteInstruction box | PostSpiteError) =>
    try
      ...
      PostSpiteError("dQ: incorrect arguments")
    end
...
class QuadTAbs is PostSpiteInstruction
  var _next: PostSpiteInstruction box = Return
  ...
  fun exec(dmachine: PostSpiteMachine): (PostSpiteInstruction box | PostSpiteError) =>
    try
      ...
      PostSpiteError("dT: incorrect arguments")
    end

class QuadTRel is PostSpiteInstruction
  var _next: PostSpiteInstruction box = Return
...
class PostSpiteMachine
  ...
  fun ref dt_abs() ? =>
    let y = pop()?
    let x = pop()?
    _path_commands.command(PathBezQuadT.abs(x, y))

  fun ref dt_rel() ? =>
    let y = pop()?
    let x = pop()?
    _path_commands.command(PathBezQuadT.rel(x, y))
...
class PostSpiteLangParser
  ...
  fun ref _process_tokens(tokens: Iterator[String]): ((PostSpiteInstruction, PostSpiteInstruction) | PostSpiteError) =>
    ...
    for t in tokens do
      ...
      elseif t == "roll" then
        inst = _add_inst(inst, Roll)
      elseif t == "pick" then
        inst = _add_inst(inst, Pick)
      elseif t == ">" then
      ...
      elseif t == "dT" then
        inst = _add_inst(inst, QuadTAbs)
      elseif t == "dt" then
        inst = _add_inst(inst, QuadTRel)
      elseif t == "stop" then
